...
static void lok_doc_view_class_init (LOKDocViewClass* pClass)
{
     ...
     * @load_event: the #LOK_DOC_VIEW_LOAD_EVENT
     *
     * Emitted when a load operation in @pDocView changes.

    doc_view_signals[LOAD_CHANGED] =
        g_signal_new("load-changed",
                     G_TYPE_FROM_CLASS (pClass),
                     G_SIGNAL_RUN_FIRST,
                     G_STRUCT_OFFSET (LOKDocViewClass, load_changed),
                     NULL, NULL,
                     g_cclosure_marshal_VOID__ENUM,
                     G_TYPE_NONE, 1,
                     LOK_DOC_VIEW_LOAD_EVENT);
     */
    /*
     * LOKDocView::load-failed:
     * @pDocView: the #LOKDocView on which the signal is emitted
     * @failing_document_path: the path of the document that failed loading
     * @error: the #GError that was triggered
     *
     * Emitted when the load operation requested for a specified document_path
     * fails.
     *
     * Returns: %TRUE to stop other handlers from being invoked, %FALSE to to
     * propagate the event further.
     */
    /*
    doc_view_signals[LOAD_FAILED] =
        g_signal_new("load-failed",
                     G_TYPE_FROM_CLASS (pClass),
                     G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET (LOKDocViewClass, load_failed),
